...
mod tests {
    ...
    fn height_select_invert_flips_selection() {
        let inputs = HashMap::from([("input".to_string(), PortValue::Heightmap(const_hm(2, 2, 0.5)))]);
        let base = &[
    ...
    fn height_select_smooth_falloff_differs_from_linear() {
        ...
        let inputs = HashMap::from([("input".to_string(), PortValue::Heightmap(const_hm(2, 2, 0.85)))]);
        let common = [
        ...
        let smooth = run_node(NodeType::HeightSelect, &sm, inputs, 2, 2)
            .get(0, 0)
            .unwrap();
        assert!((lin - 0.3).abs() < 1e-4, "linear ramp expected 0.3, got {lin}");
        assert!((smooth - 0.216).abs() < 1e-3, "smooth ramp expected 0.216, got {smooth}");
    }
}
