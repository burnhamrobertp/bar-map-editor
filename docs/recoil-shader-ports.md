...
| `SMFVertProg.glsl`, `SMFFragProg.glsl`         | **Mostly ported**            | `shaders/recoil/smf_ground.wgsl` + `shaders/terrain.wgsl` | Detail tex, splat detail-normal (color + normal perturb), sky cube reflection, water absorb, shadows, spec-add-not-multiply all in. Remaining: parallax, SMF_BLEND_NORMALS, infotex. |
| `BumpWaterVS.glsl`, `BumpWaterFS.glsl`         | **Mostly ported**            | `shaders/water.wgsl`               | Octave normals, surface composite, refraction with depth mixback, fresnel reflection, sun spec, height fog overlay, tonemap all in. Remaining: shore foam, caustics, multi-tap reflection blur. |
...
| `ModelVertProg.glsl`, `ModelFragProg.glsl`     | **Partially ported**         | `shaders/features.wgsl`            | Diffuse + SMF-style lighting + shadow + texture2.r emissive + texture2.g specular multiplier (`MFP:87-109`). Remaining: env-cubemap reflection (mixed via `texture2.g`), team-colour replacement via `texture1.a`. |
