...
| `BumpWaterVS.glsl`, `BumpWaterFS.glsl`         | **Mostly ported**            | `shaders/water.wgsl`               | Octave normals, surface composite, refraction with depth mixback + engine-formula UV distortion, 7-tap reflection blur (`opt_blurreflection`) with mapinfo-driven `blurBase`/`blurExponent`, fresnel reflection, sun spec, height fog overlay, tonemap, 32-frame caustic animation (loaded from engine `bitmaps.sdz`) all in. **Remaining**: shore foam (`GetShorewaves`). Needs CPU coast-distance transform bake from the heightmap + coastmap texture binding + multiple new uniforms (`WaveFoamDistortion`, `WaveOffsetFactor`, `InvWavesLength`) + complex multi-channel foam-blend math (`BumpWaterFS:186-220`). Asset loader for `foam.jpg` + `shorewaverand.png` is already in place (`bar_data::water_assets`); deferred work is the coastmap bake + shader stage. |
