...
- **Map-border: full engine port + detail-texture overlay.** The minimal engine-faithful skirt landed: edge-clamped albedo sample × `diffuseMult = 110/255` darkening + heightmap-derived alpha fade approximated via `discard` (since the terrain pipeline is opaque). Remaining work to fully match `SMFBorderFragProg.glsl`: (a) add the detail-texture overlay (`detailsCol = texture(detailsTex, vDetailsUV) * 2 - 1` plumbed into the same skirt branch), (b) replace the discard-threshold with true alpha blending so the skirt fades smoothly rather than cutting off (likely a separate pass for the skirt strip after the opaque terrain pass), and (c) source the border `detailsUV` per the engine's `detailPlaneS/T` projection rather than the current heightmap-clamped UV. Also worth verifying against an in-engine capture once the alpha-blend path is in.
